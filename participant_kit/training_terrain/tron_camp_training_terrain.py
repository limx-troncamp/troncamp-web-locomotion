# TronCamp (Tron2ATask, TRON2) 训练地形 —— 随机化、宽阔开放，覆盖评测的全部地形要素。
#
...
# TRON2A 站高略低于 1 m；把 base 放在距地格原点上方约 0.97 m（与评测环境一致）。
TRON2_SPAWN_Z = 0.966

# 比 3 m 的评测走廊更宽，让学习中的策略有横向回旋余地。
TRACK_WIDTH = 8.0

# TRON2A（腿式 / 轮足）能应对完整的障碍范围 —— 从 builder 默认值起步。
_track = MixedTrackCfg()
_track.step_height_range = (0.10, 0.18)
_track.platform_height_range = (0.15, 0.55)

# 10 x 10 = 100 条各自独立随机化的赛道（难度随行递增）。按你的 GPU 调整。
TASK_C_TRAINING_TERRAIN_CFG = make_training_terrain_cfg(
    width=TRACK_WIDTH,
    cell_length=80.0,
    num_rows=10,
    num_cols=10,

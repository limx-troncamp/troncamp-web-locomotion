...
class AlgSolution:
    ...
    def get_action_spec(self) -> dict[str, dict[str, Any]] | None:
        ...
        逐关节 PD 覆盖（想复现训练步态时把训练 PD 一并声明）——列表须按机器人固定关节顺序、长度=该组关节数。
        TRON2 leg 有 10 个关节（sfyg；wfyg 无踝为 8），顺序与推荐值见指南 §4「PD 覆盖示例」，下为评测默认值，
        照抄再改成你的训练 PD 即可：
            return {"leg": {
                "mode": "position", "scale": 0.5,
                "stiffness": [159.67, 159.67, 53.22, 159.67, 53.22,
                              159.67, 159.67, 53.22, 159.67, 53.22],
                "damping":   [10.16, 10.16, 3.39, 10.16, 3.39,
                              10.16, 10.16, 3.39, 10.16, 3.39]}}
        Oli 只有一组 "leg" 且涵盖全部 31 个关节，声明 31 维列表（按 OLI_EDU_JOINT_NAMES 顺序；推荐值见指南 §4）：
            return {"leg": {"stiffness": [...31...], "damping": [...31...]}}
        没填的分组 / 没填的键都保持默认 PD；未声明本方法则整体用官方默认。
        """
        return None

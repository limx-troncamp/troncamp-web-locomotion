"""TronCamp (Tron2Task) / HumanoidCamp (OliTask) 基准的参考 `solution.py` 模板。
...
# （Oli/OliTask：proprio 106 维、固定头部 13（含 base_lin_vel + 4 维速度指令 vx,vy,wz,stand_flag），
...
class AlgSolution:
    ...
    def predicts(self, obs, current_score) -> dict:
        ...
        #   是展平的一维向量 channels*360，reshape 成 (channels, 360) 得逐环结构。
